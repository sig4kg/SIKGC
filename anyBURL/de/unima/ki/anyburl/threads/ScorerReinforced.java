...
public class ScorerReinforced extends Thread {
	...
	public void run() {
		...
		boolean done = false;
		while (done == false) {
			if (!LearnReinforced.active(this.id, this.storedRules, this.createdRules, this.producedScore, this.mineParamZero, this.mineParamCyclic, this.mineParamAcyclic, this.mineParamLength) || !ready) {
				this.createdRules = 0;
				this.storedRules = 0;
