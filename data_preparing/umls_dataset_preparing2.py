...
def get_types(all_triples, cid2phrase, cid2types, work_dir):
...
def convert2abox_nt(work_dir, text2Obj):
...
def check_ent2type(ent2type_file, type2text_file):
    ent2type = dict()
    type2text = {}
    with open(ent2type_file, 'r') as f:
        lines = f.readlines()
        for line in lines:
            items = line.strip().split('\t')
            ent2type.update({items[0]: items[1].split(';')})
    with open(type2text_file, 'r') as f:
        lines = f.readlines()
        for line in lines:
            items = line.strip().split('\t')
            type_phrase = items[1]
            type_iri = items[0]
            type2text.update({type_iri: type_phrase})
    for ent, types in ent2type.items():
        print(f"{ent}: {';'.join([type2text[t] for t in types])}")
...
if __name__ == "__main__":
    ...
    # convert2abox_nt("../resources/UMLS/", phrase2typeobj)
    # fix_tmp("../resources/UMLS_full/", "../resources/UMLS/")
    check_ent2type("../resources/UMLS/entity2type.txt", "../resources/UMLS_full/entity2text1.txt")
