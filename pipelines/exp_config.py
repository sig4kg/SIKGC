class BLPConfig:
    ...
    def get_blp_config(self, rel_model, inductive, dataset, schema_aware, silver_eval, do_produce):
        ...
        if dataset == "DBpedia":
            ...
            tmp_conf.update({'lr': 1e-3, 'regularizer': 1e-2, 'max_epochs': 2, 'batch_size': 64})
            if inductive:
                tmp_conf.update({'model': 'fasttext', 'lr': 5e-4, 'max_epochs': 2, 'batch_size': 128})
        elif dataset == "NELL":
            tmp_conf.update({'max_epochs': 80})
            if inductive:
                tmp_conf.update({'lr': 1e-4, 'batch_size': 1024, "max_epochs": 80})
            if rel_model == 'rotate':
