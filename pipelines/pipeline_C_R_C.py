...
from abox_scanner.abox_utils import ContextResources, TBOX_PATTERNS_PATH, ORIGINAL_TRIPLES_PATH, read_scanned_2_context_df
import pandas as pd
...
def c_r_c(input_hrt_triple_file, work_dir, max_epoch=2):
    ...
    abox_utils.wait_until_file_is_saved(f"{work_dir}valid_hrt.txt", 60)
    ...
    for ep in trange(max_epoch, colour="green", position=0, leave=True, desc="Pipeline processing"):
        ...
        if abox_utils.wait_until_file_is_saved(work_dir + "valid_hrt.txt", 60):
        ...
        hrt_int_df_2_hrt_rumis(train_hrt_df, context_resource, work_dir + "idea.data.txt")
