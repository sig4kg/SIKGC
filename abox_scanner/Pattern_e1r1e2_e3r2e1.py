...
class Pattern_e1r1e2_e3r2e1(PatternScanner):
    ...
    def scan_pattern_df_rel(self, triples: pd.DataFrame, log_process=True):
        ...
        for g in tqdm(gp, desc="scanning pattern e1r1e2_e3r2e1 ", disable=not log_process):
            r1 = g[0]
            if r1 in self._pattern_dict:
                ...
                df.update(r1_triples_df.query(f"head in @tmp_list and is_new==True")['is_valid'].apply(lambda x: False))
        return df
