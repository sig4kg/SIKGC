...
def cacmclc(work_dir, input_dir, schema_file, tbox_patterns_dir, loops=1, schema_in_nt='', exclude_rels=[], blp_config={}):
    ...
    for ep in trange(loops, colour="green", position=0, leave=True, desc="Pipeline processing"):
        ...
        init_c1, extend_c1, nc1, nv1, ncc1 = LC_block(context_resource,
                                                      ...
                                                      exclude_rels=exclude_rels)
        s = get_scores(init_c3, extend_c1 + context_resource.new_type_count, nc1 + nc2 + nc3, nv1 + nv2 + nv3, ncc1 + ncc2 + ncc3)
        scores.append(s)
