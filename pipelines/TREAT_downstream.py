...
def TREAT_for_downstream_task(work_dir):
    ...
    split_all_triples(context_resource, work_dir, inductive=False,
    ...
    to_scan_df = pd.concat([context_resource.hrt_int_df, pred_hrt_df]).drop_duplicates(keep="first").reset_index(
        drop=True)
    abox_scanner_scheduler.set_triples_to_scan_int_df(to_scan_df).\
        scan_IJ_patterns(work_dir=work_dir, save_result=True)
