...
public class Predictor extends Thread {
	...
	public Predictor(TripleSet testSet, TripleSet trainingSet, TripleSet filterSet, int k, HashMap<String, ArrayList<Rule>> relation2Rules4Prediction) {
	...
	public void run() {
		...
		while (triple != null) {
			...
			if (Settings.AGGREGATION_ID == 1) {
				RuleEngine.predictMax(testSet, trainingSet, filterSet, k, relation2Rules4Prediction, triple);
			}
			else if (Settings.AGGREGATION_ID == 2) {
				RuleEngine.predictNoisyOr(testSet, trainingSet, filterSet, k, relation2Rules4Prediction, triple, true);
				RuleEngine.predictNoisyOr(testSet, trainingSet, filterSet, k, relation2Rules4Prediction, triple, false);
			}
