...
def read_hrt_pred_anyburl_2_hrt_int_df(pred_anyburl_file, context_resource: ContextResources):
    with open(pred_anyburl_file) as f:
        ...
        for chunk in chunks:
            ...
            for ph in pred_hs:
                tmp_preds.append([ph[0], r, t])
            for pt in pred_ts:
                tmp_preds.append([h, r, pt])
...
def split_all_triples(anyburl_dir):
    ...
    sample_dev.to_csv(osp.join(anyburl_dir, f'dev.txt'), header=False, index=False, sep='\t')
...
def prepare_anyburl_configs(anyburl_dir):
    config_apply = f"PATH_TRAINING  = {anyburl_dir}train.txt \n" \
                   ...
                   f"PATH_OUTPUT    = {anyburl_dir}predictions/alpha-|default| \n" \
...
def wait_until_anyburl_data_ready(work_dir, inductive=False):
    if inductive:
...
if __name__ == "__main__":
    read_hrt_pred_anyburl_2_hrt_int_df("../outputs/alpha-100", None)
